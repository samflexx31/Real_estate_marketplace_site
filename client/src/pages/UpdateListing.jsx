  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setImageUploadError("Image upload failed (2 mb max per image)", true);
          setUploading(false);
        });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (+formData.regularPrice < +formData.discountPrice) {
        return setError("discount price muust be lower than regular price");
      }

      setLoading(true);
      setError(false);
      const res = await fetch(`/server/listing/update/${params.id}`, {
        method: "POST",
        headers: {
